...
## ADR-014 amendment (2026-09-11, docs/qa/RELAY_VERIFICATION.md) | Audio transcode is mandatory; shipped topology = one hook per session
- MediaMTX `forward` is pass-through only: against a legacy RTMP server the Opus track is silently DROPPED (video continues, no sound); against Enhanced-RTMP peers Opus is forwarded but YouTube/Twitch cannot decode it. Verified both ways on host.
- Shipped design (infra/relay): per session, `runOnAvailable` runs one FFmpeg `-c:v copy -c:a aac` + `tee` (`use_fifo=1`, `fifo_options=attempt_recovery=1`, `onfail=ignore`) to all destinations; 9:16 destinations get a scaling variant. Cost 12-15% of one core vs 3% pass-through. Destination kill/restart rejoined without disturbing siblings (verified). The two-stage variant (hook -> internal path -> native forward) also passed and remains an option if per-destination add/remove without restarting the hook becomes necessary.
- Config corrections vs stale docs: `webrtcLocalUDPAddress` (not `webrtcICEUDPMuxAddress`), permission path regex `~^live/.*$`, path removal uses HTTP DELETE.
- Docker packaging UNVERIFIED on host (no nested virtualisation for WSL2); the same mediamtx.yml was verified with the checksum-matched native binary.

## ADR-016 | ACCEPTED (2026-09-12, directive docs/prompt-pack/10) | Public experience = "Live surface" scroll grammar built on the real product surface
- The public page at `/` is rebuilt as an interactive product experience: Scroll Craft "Live surface" grammar (the page behaves like the product; scroll and the visitor drive real state computed from labelled sample data), Anime.js 4 for choreography, the Scroll Craft engine for pinned acts (`--sc-p`), one bespoke signature move (drag a LIVE destination off the stage; it isolates and reconnects while siblings stay live).
- Honesty rule: every playground computes state with real logic (the same state machine and intent/format rules as packages/core, run in the page against sample data) and the page says on its face that the scenario is a demo. No painted dashboards, no invented statistics, no scroll cues, no section counters.
- Site and app share tokens, type, icons, motion and terminology (packages/ui CSS reused verbatim). ACT 8 hands off into the real onboarding (`/app/start`).
- Constraints: transform/opacity-only motion, reduced-motion alternative for every interaction, mobile as its own composition, public page JS budget 60 KB gzipped including anime.js and the engine.

## ADR-017 | ACCEPTED (2026-09-14, directive docs/prompt-pack/11, audit docs/qa/LIVETAP_FIRST_TIME_CREATOR_AUDIT.md) | Public experience rebuilt picture-first: fixed band layer, no scroll-gated content, nothing goes live by itself
- The first-time-creator audit (59/150) found the deployed page introducing itself as broken: a scroll-driven intro collage over the headline, a dead mouse wheel, pale narration sliding through the seams between sticky stages, an empty stage, silent controls and a Download that led nowhere. The dead wheel could not be reproduced in Chromium with real wheel events and the old page registered no wheel or touch listener; the audit machine's Chrome could not be scripted on any page, so it gave no evidence. What the old page verifiably had was everything a visitor needs depending on scroll progress, plus continuous rAF and animated-layer work, and those are what this decision removes.
- Decisions: (1) the Live-surface grammar (ADR-016) stays, but the intro lattice, the atmosphere canvas, the grain layer, the ground drift, scroll counters and reveals are removed: nothing a visitor needs waits for scroll or for a frame loop. (2) Chapter copy lives in ONE fixed band layer, one band per chapter, cross-faded in place and switched by a single passive scroll listener (the act owning the viewport's 45% line); bands never move, never clip, never overlap the surface. (3) The stage shows a picture from first paint: generated sample creator footage, a guest clip and a screen asset, and "Use my camera" puts the visitor's own camera on the stage locally (mirrored self-view, never uploaded, denial and insecure-context paths explained). One compose() draws the same production into six platform-shaped output canvases and into every connected tile. (4) The guided demo lasts 4.2 s and stops at READY with "Your turn. Tap GO LIVE." written under the button; the page never goes live on its own. (5) The peak ("Break it yourself") is chapter two with a one-tap path in addition to drag and keyboard. (6) The silent-control rule: every control works at every scroll position and every state, or explains itself next to itself; GO LIVE with nothing picked connects the intent's two suggested destinations and says so. (7) No Download while no build exists; "Watch on GitHub for the first build" plus an early-access endpoint that only renders a form when the owner configures an endpoint (B-009). (8) Phones are their own composition: Vertical Live and 9:16 by default, tall picture, tile strip, GO LIVE; Shapes and Simple/Pro live in their chapters' bands.
- Budget outcome: landing JS 53 KB gz (limit 60), CSS 15 KB gz, footage 360 KB total, 0 long tasks in 5 idle seconds, 61 fps on the build host.
