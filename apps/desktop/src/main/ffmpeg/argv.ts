...
export function buildSenderArgv(spec: SenderArgvSpec): string[] {
  ...
  const argv: string[] = ['-hide_banner', '-nostdin', '-loglevel', 'error'];
  if (spec.progress !== false) argv.push('-stats_period', '1', '-progress', 'pipe:2');
  argv.push('-fflags', '+nobuffer', '-f', 'mpegts', '-i', 'pipe:0', '-map', '0');

  switch (spec.ingest.protocol) {
    case 'rtmp':
    case 'rtmps': {
      const url = composeRtmpPublishUrl(spec.ingest);
      assertCleanToken(url, 'Stream URL');
    ...
    case 'srt': {
      ...
      assertCleanToken(url, 'SRT URL');
      argv.push('-c', 'copy', '-f', 'mpegts', url);
      return argv;
    }
    case 'whip': {
      assertCleanToken(spec.ingest.url, 'WHIP endpoint');
...
export function buildSrtUrl(ingest: IngestTarget): string {
  assertCleanToken(ingest.url, 'SRT URL');
  const params: string[] = [];
...
export function buildRecordingArgv(spec: RecordingArgvSpec): string[] {
  ...
  const argv: string[] = ['-hide_banner', '-nostdin', '-loglevel', 'error'];
  if (spec.progress !== false) argv.push('-stats_period', '1', '-progress', 'pipe:2');
  argv.push('-f', 'mpegts', '-i', 'pipe:0', '-map', '0', '-c', 'copy');
  if (spec.container === 'mp4') {
    argv.push('-movflags', '+frag_keyframe+empty_moov+default_base_moof', '-f', 'mp4');
  } else {
...
export function buildTeeOutput(slaves: TeeSlave[]): string {
  ...
  return slaves
    .map((slave) => {
      ...
      // Local file paths are allowed here, so use the path-level check then escape for tee.
      assertCleanPath(slave.url, 'tee slave URL');
      const opts = [`f=${slave.format}`];
      if (slave.onfail) opts.push(`onfail=${slave.onfail}`);
      if (slave.useFifo) opts.push('use_fifo=1', 'fifo_options=attempt_recovery=1\\:recover_any_error=1');
